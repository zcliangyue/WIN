...
from model.components.swin_transformer_v2 import SwinTransformerBlockV2, PatchMergingV2
...
class WindowAttention(nn.Module):
    def __init__(self, dim: int, window_size: int, num_heads: int, qkv_bias: Optional[bool] = True,
        ...
        coords = torch.stack(torch.meshgrid([coords_size_h, coords_size_w]))
...
class TULIP(nn.Module):
    def __init__(self, img_size = (32, 2048), target_img_size = (128, 2048) ,patch_size = (4, 4), in_chans: int = 1, embed_dim: int = 96,
        ...
        #self.upscale_factor = int(((target_img_size[0]*target_img_size[1]) / (img_size[0]*img_size[1])))#**0.5 * 2 * ((patch_size[0]*patch_size[1])//4**0.5))
    ...
    def forward(self, x, mc_drop = False):
